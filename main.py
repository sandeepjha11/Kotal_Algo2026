...
class TradingApp:
    ...
    def on_message(self, message):
        try:
            ...
            for tick in data:
                ...
                # 🔎 Debugging tip: print every tick
                print("Updating", token, "with", ltp)

                if token in self.token_label_map and ltp is not None:
                    try:
                        new_text = f"{float(ltp):.2f}"
                        self.token_label_map[token].set(new_text)   # update StringVar
                        print("Label for", token, "now shows:", self.token_label_map[token].get())
                    except ValueError:
    ...
    def update_index_prices(self):
        """Placeholder until WebSocket ticks fully replace polling."""
