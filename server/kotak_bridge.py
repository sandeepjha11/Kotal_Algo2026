...
def get_cached_scrip(segment):
    now = time.time()
    if segment in scrip_cache:
        df, timestamp = scrip_cache[segment]
        if now - timestamp < 3600:  # Cache for 1 hour
            return df

    logger.info(f"Downloading scrip master for segment: {segment}")
    url = client.scrip_master(exchange_segment=segment)
    df = pd.read_csv(url)
    df.columns = [c.strip() for c in df.columns.values.tolist()]
    scrip_cache[segment] = (df, now)
    return df
...
def get_spot():
    ...
    try:
        ...
        if filtered.empty:
            logger.error(f"Spot symbol {symbol} not found in {segment} scrip master")
            return jsonify({"status": "error", "message": f"Spot symbol {symbol} not found"}), 404
        ...
        if quote.get('stat') != 'Ok':
            logger.error(f"Quote API failed for {symbol}: {quote.get('emsg')}")
        ...
        logger.error(f"Error in get_spot for {symbol}: {str(e)}")
        return jsonify({"status": "error", "message": str(e)}), 500
